...
export async function POST(req: NextRequest) {
  try {
    ...
    // Role check: only ADMIN or STAFF can generate reports
    ...
    const { hospitalId } = body as { hospitalId: number };
    ...
    return NextResponse.json({
      pdf: pdfBase64,
      filename: `Oxyrious_Report_${hospital.name.replace(/\s+/g, "_")}_${monthYear.replace(/\s+/g, "_")}.pdf`,
